...
def write_topology_anynet(technology, chiplets, placement, topology, packaging, booksim_config, ici_graph, link_lengths_internal, path):
...
def run_booksim_simulation_sub(technology, chiplets, placement, topology, packaging, booksim_config, ici_graph, topology_path, name, precision, saturation_factor, traffic_mode, timeout = None):
	...
	while True:
		...
		if timeout == None:
			try:
				out = subprocess.check_output(['booksim2/src/booksim', config_path], stderr=subprocess.PIPE)
			...
			try:
				out = subprocess.check_output(['booksim2/src/booksim', config_path], stderr=subprocess.PIPE, timeout = timeout)
			except subprocess.TimeoutExpired:
				return results
			except subprocess.CalledProcessError:
				pass
		...
		# We don't do logging to avoid filling up the file system
		"""
			...
			for (line_idx, line) in enumerate(result_lines):
				for metric in metrics:
					if (metric + " average") in line:
						...
						results[load][key]["max"] = float(result_lines[line_idx+1].split(" ")[2])
...
def run_booksim_simulation(technology, chiplets, placement, topology, packaging, booksim_config, ici_graph, link_lengths_internal, name, timeout = None):
	# Write the topology.anynet file
	topology_path = "booksim2/src/topologies/%s.anynet" % name 
	write_topology_anynet(technology, chiplets, placement, topology, packaging, booksim_config, ici_graph, link_lengths_internal, topology_path)
	...
	precision = 0.001
	saturation_factor = 2
	return run_booksim_simulation_sub(technology, chiplets, placement, topology, packaging, booksim_config, ici_graph, topology_path, name, precision, saturation_factor, booksim_config["traffic_mode"], timeout)
...
if __name__ == "__main__":
	...
	parser.add_argument("-to", "--time_out", required = False, help = "Timeout for BookSim")
	args = parser.parse_args()
	timeout = int(args.time_out) if args.time_out != None else None
	...
	# Run the simulation
	results = run_booksim_simulation(technology, chiplets, placement, topology, packaging, booksim_config, ici_graph, link_lengths_internal, args.results_file, timeout)
	# Store results
